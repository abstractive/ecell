class Ef::Service::Webstack::WebSocket

  include Ef::Pack::Extensions, Ef::Service::Webstack::Extensions
  ...
  def emitters!
    ...
    if handshake.valid?
      ...
      response.render(@io)
      debug(message: "#{[:open, @uuid]}", reporter: self.class) if DEBUG_SERVICES

      console(message: "sending hello message through recently opened socket", reporter: self.class) if DEBUG_SOCKET
      send "Hello from the EnergyFlow message server."
      send "CLIENT ID: #{@uuid}"
    ...
    on_close { |status, reason|
      debug(message: "#{[:close, @uuid, status, reason]}", reporter: self.class) if DEBUG_SERVICES
    ...
    on_ping { |payload|
      debug(message: "#{[:ping, @uuid, payload]}", reporter: self.class) if DEBUG_SERVICES
      reset_inactivity_timer
      write ::WebSocket::Message.pong(payload).to_data
    }

    on_pong { |payload|
      debug(message: "#{[:pong, @uuid, payload]}", reporter: self.class) if DEBUG_SERVICES
      reset_inactivity_timer
    }

    on_error { |message|
      console(message: "hit an error: #{message}", reporter: self.class) if DEBUG_SERVICES
      close!
    }

    # publish the received message
    on_message { |message|
      debug(message: "#{[:message, @uuid, message]}", reporter: self.class) if DEBUG_SERVICES
      if !message.empty?
        begin
          if DEBUG_SERVICES
