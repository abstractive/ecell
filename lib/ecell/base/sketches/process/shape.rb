  ...
  module Base
    module Sketches
      class ProcessShape < ECell::Elements::Figure
        ...
        def on_started
          initialize_line(:distribution_tasks_push2, mode: :binding, provision: true)
          initialize_line(:distribution_events_push2, mode: :binding, provision: true)
        ...
        def on_running
          ...
          @check_process = every(INTERVALS[:report]) {
            tasks = 0 #de ECell.sync(:calling).call_sync(:tasks).count
            events = 0 #de ECell.sync(:calling).call_sync(:events).count
            debug("Sending message to :webstack.", tag: :reporting)
            ECell.sync(:calling).call_sync(:webstack).announcement(rpc: {
              tag: :report,
