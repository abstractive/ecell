...
module ECell
  module Base
    module Shapes
      class Spool < ECell::Elements::Figure
        def initialize
          #benzrf TODO: proper require for this
        ...
        def forward(figure_id)
          @timers[figure_id] ||= after(1) {
            if alive?(figure_id)
              @backlog[figure_id].each { |command|
                ECell.sync(figure_id).async(command[:method], *command[:args])
              }
              @backlog[figure_id].flush
              @timers[figure_id].cancel rescue nil
              @timers[figure_id] = nil
            else
              forward(figure_id)
            end
          }
        ...
        def alive?(figure_id)
          ECell.sync(figure_id) && ECell.sync(figure_id).alive?
        rescue
          false
        end
